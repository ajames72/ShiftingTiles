	...
	var PictureTileView = Backbone.View.extend({
		...
		render: function(slotIndex){
			...
			*/
			...
			var FP = new FullscreenPositioner(this.model.get('imgProperties').width, this.model.get('imgProperties').height, this.model.get('imgProperties').margin, this.model.get('imgProperties').margin, this.$el.children('img.cell-1'));
	    FP.fitCenter(this.$el.children('img.cell-1'));
