...
/*jshint esnext: true */
define(['jQuery', 'underscore', 'backbone', '../models/PictureTileModel', '../views/PictureTileView', '../views/PictureTileRowView', '../views/PictureTileDescriptionView'], function($, _, Backbone, PictureTileModel, PictureTileView, PictureTileRowView, PictureTileDescriptionView){
	/**
	...
	var PictureTileCollection = Backbone.Collection.extend({
		...
		setTransitionTime: function(val){
			this.transitionTimer = val || 2000;
		},
		/**
		 *
		 * @param none
			...
			switch(this.loading){
				case LOADING_SETTINGS[1]:
					...
					if(this.slots.length > 1){
						var newSlotIndex;
						do {
							newSlotIndex = getRandomInt(0, this.slots.length);
						} while(newSlotIndex === this.removeSlotIndex);
						this.removeSlotIndex = newSlotIndex;
					} else {
		 ...
		 * @description Initialises the available image slots with image indexes.
		 * @param none
		 * @returns none
		 */
		initializeSlots: function(index){
			...
			pictureTileView.style = this.style.tile;
			/*
			...
			*/
			/*
			var FP = new FullscreenPositioner(imgProperties.width, imgProperties.height, imgProperties.margin, imgProperties.margin, pictureTileView.$el);
	    FP.fitCenter(pictureTileView.$el);
			*/
			this.slots.push(pictureTileView);
		},
		/**
		 * @description This renders the next image
		...
		renderNext: function(self){
			...
			//render image
			self.slots[self.removeSlotIndex].model.set({
				imgSrcIndex: nextModelFromCollection.get('imgSrcIndex'),
				src: nextModelFromCollection.get('src'),
				id: nextModelFromCollection.get('id'),
				description: nextModelFromCollection.get('description'),
				imgProperties: nextModelFromCollection.get('imgProperties')
			});
			//render description
			//self.descriptionView.model.set('description', nextModelFromCollection.get('description'));
		},
		/**
		...
		render: function(){
			...
			if(this.viewSize <= this.size()){
				...
				console.log("AJMEBC ERROR: less images than available image cells.");
			}
			...
			if(this.slots.length > 0){
					...
					//this.descriptionView = new PictureTileDescriptionView({model: this.slots[0].model});
					//this.el.append(this.descriptionView.el);
					window.setInterval(this.renderNext, this.transitionTimer, this);
			} else {
				console.log("AJMEBC ERROR: no image cells available.");
			}
		...
		preload: function(data){
          ...
          success: function (_data, textStatus, jqXHR) {
						...
						image.addEventListener("load", function(){
							deferredObject.resolve({width: this.naturalWidth, height: this.naturalHeight});
						});
