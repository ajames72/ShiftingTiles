...
define(['jQuery', 'underscore', 'backbone', '../models/PictureTileModel', '../views/PictureTileView', 'text!../templates/PictureTileRowTemplate.html'], function($, _, Backbone, PictureTileModel, PictureTileView, PictureTileRowTemplate){
	...
	var PictureTileCollection = Backbone.Collection.extend({
		 ...
		 * @returns none
		 */
		initializeSlots: function(row){
			var pictureTileView;

			pictureTileView = new PictureTileView({model: this.getNextModel(), el: $(row)});

			pictureTileView["width"] = this.templateWidth;
		...
		renderNext: function(self){
			...
			self.slots[self.removeSlotIndex].model.set({imgSrcIndex: nextModelFromCollection.get('imgSrcIndex'), src: nextModelFromCollection.get('src')});
		},
		/**
			...
			if(this.viewSize <= this.size()){
				for(var index = 0; index < this.viewSize; index++){
					...
					if((index === 0) || ((index % self.columns) === 0)){
						rowIndex++;
						rowSelector = "div.ajmebc-shifting-tile-row:nth-child("+ rowIndex +")";
					}
					//Loads image indexes into slot array
					this.initializeSlots(rowSelector);
